/**
 * 与 Vite 中间件、Vercel Serverless 共用的聊天 API 核心逻辑
 */
...
export function buildMcpToolsPayload(chatEndpoint: string) {
  return {
...
export async function runChatRequest(
  ...
  if (!cfg) {
    return { status: 400, json: { error: '不支持的 provider' } }
  }

  const userPrompt = (body.prompt ?? '').trim()
  if (!userPrompt) {
    return { status: 400, json: { error: 'prompt 不能为空' } }
  }
  ...
  if (!apiKey) {
    ...
    return {
      ...
      json: {
        ...
        detail: [
          '请在 Vercel 项目 Environment Variables（或本地 .env）中配置对应密钥。',
          `${cfg.envKey}=你的密钥`,
          '若仅需离线演示，请勿设置 DISABLE_AI_MOCK=1。',
        ].join('\n'),
      },
    }
  }
